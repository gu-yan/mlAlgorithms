...
from inception_impl import inceptionV1_parameters
...
tf.app.flags.DEFINE_bool('train_mode', False, 'run in train mode or eval modo.')
...
def pooling(data, ksize, strides, name, padding='SAME', type='max'):
    with tf.name_scope(name):
        if type == 'max':
            ...
            return tf.nn.avg_pool(value=data, ksize=[1, ksize, ksize, 1], strides=[1, strides, strides, 1],
    ...
    with tf.name_scope(name):
        ...
        if train_flag:
            with tf.name_scope(name + 'patch5'):
                ...
                drop = tf.nn.dropout(patch5_fc1, keep_prob=KEEP_PROB)
                patch5_fc2 = fullconnection(data=drop,
...
def model(input, labels, model_param, train_mode=True):
    ...
    avage_pool1 = pooling(data=inception9[0],
                          ...
                          type='avage')
    avage_pool1 = tf.reshape(avage_pool1,
                             shape=[-1, model_param['fc1_weights'].get_shape()[0].value])
...
def train_act(features_train, labels_train, features_test, labels_test):
    ...
    model_paramters = inceptionV1_parameters.init_param(NUMBER_CLASSES)
